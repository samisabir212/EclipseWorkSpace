...
public class Using_PathParameter {
	
	// https://api.twitter.com/1.1/statuses/show.json?id=210462857140252672

		/*
		 * HERE I AM USING A PATH PARAMTER INSTEAD OF QUERY PARAMTER WHEN IM DELETING A TWEET
		 * 
		...
		public void postTWEET() {
			Response response = given()
					...
					.queryParam("status", "im posting this tweet to read it then i want to delete the tweet").when()
					.post("/update.json")
		...
		public void deleteTweet() {
			...
			.auth()
			.oauth(consumerKey, consumerSecret, accessToken, accessSecret)
			.pathParam("id", tweet_ID2)
			.when()
			.post("/destroy/{id}.json") //the curly braces is going to be replaced with the pathParam(tweet_id2)== ID
			.then().statusCode(200);
